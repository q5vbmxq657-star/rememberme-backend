...
def install_avatar_runtime_livekit_safety_patch() -> None:
    """
    Install the RememberMeAI output class into the module namespace used by the
    Tavus plugin.

    The operation is idempotent and performs no network operation.
    """
    ...
    import livekit.agents.voice.avatar as avatar_public_module
    import livekit.plugins.tavus.avatar as tavus_avatar_module

    avatar_public_module.DataStreamAudioOutput = (
    ...
    logger.info(
        "RememberMeAI deterministic LiveKit shutdown patch installed."
    )


def safety_patch_status() -> dict[str, Any]:
    import livekit.agents.voice.avatar as avatar_public_module
    import livekit.plugins.tavus.avatar as tavus_avatar_module

    return {
        "installed": _PATCH_INSTALLED,
        "public_avatar_output_patched": (
            avatar_public_module.DataStreamAudioOutput
            is RememberMeDataStreamAudioOutput
        ),
        "tavus_avatar_output_patched": (
            tavus_avatar_module.DataStreamAudioOutput
            is RememberMeDataStreamAudioOutput
        ),
        "implementation": (
            f"{RememberMeDataStreamAudioOutput.__module__}."
            f"{RememberMeDataStreamAudioOutput.__name__}"
        ),
        "deterministic_interrupt_drain": True,
        "atomic_disconnect_barrier": True,
    }
