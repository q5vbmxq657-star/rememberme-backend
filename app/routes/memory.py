...
from app.schemas.memory import MemoryChatRequest, MemoryChatResponse, MemoryItem
...
def retrieve_backend_memories(request: MemoryChatRequest):
    ...
    try:
        ...
        search_response = vector_service.search(
            SearchMemoryRequest(
                ...
                limit=max(10, request.retrieval_limit)
            )
        )
        ...
        retrieved = [
            ...
            for row in filtered[:max(1, request.retrieval_limit)]
        ]
        ...
        if debug:
            print("⚠️ No retrieved memories above threshold. Falling back to provided memories.")

        return request.memories

    except Exception as error:
        print("⚠️ Backend retrieval failed, falling back to provided memories:", error)
        return request.memories
...
def memory_chat(request: MemoryChatRequest):
    try:
        ...
        enriched_request = MemoryChatRequest(
        ...
        return service.generate_response(enriched_request)
